...
//           'http://10.0.2.2:105/hasil_sounding_admin',
//           {
//             method: 'POST',
//             headers: {
...
//           Alert.alert(
//             'Berhasil',
//             `File PDF '${data.message}' berhasil dihasilkan dan didownload`,
//           );
//         } else {
...
//           `http://10.0.2.2:105/get_data_by_bulan_s?bulan=${bulan}`,
//         );
...
//           const options = {
//             html: htmlContent,
//             fileName: `hasil_sounding_${bulan}`,
//             directory: 'Documents',
//           };
...
//           Alert.alert(
//             'Berhasil',
//             `File PDF 'hasil_sounding_${bulan}.pdf' berhasil dihasilkan.`,
//           );

//           // Bagikan file PDF ke aplikasi lain
//           const shareOptions = {
//             title: `Hasil Sounding ${bulan}`,
...
//         Alert.alert(
//           'Error',
//           'Terjadi kesalahan saat mengonversi dan menyimpan ke PDF',
//         );
//       }
//     };

//     const handleDownloadPDF = async itemValue => {
//       downloadPDF(itemValue);
//     };
//   };
...
//             Apakah Anda Ingin Mendownload Data?
//           </Text>
...
//           <Button
//             label="Download PDF" onPress={()=>handleDownloadPDF(handleValueChange(selectedMonth))}
//           />
//         </View>
//       </View>
//     </>
...
const Download_A = () => {
  ...
  const downloadHasilSounding = async (bulan) => {
    try {
      const response = await fetch('http://10.0.2.2:105/hasil_sounding_admin', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const downloadPDF = async (bulan) => {
    try {
      const response = await axios.get(`http://10.0.2.2:105/get_data_by_bulan_s?bulan=${bulan}`);
      const data = response.data.data;
      console.log('DATA TO LOAD::11111111', response?.data); // Mengakses data dari respons menggunakan response.data
      ...
      if (response?.data?.error === 'false') {
        ...
        // Tampilkan pesan sukses
        Alert.alert('Berhasil', `File PDF 'hasil_sounding_${bulan}.pdf' berhasil dihasilkan.`);
