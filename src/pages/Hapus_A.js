...
const Hapus_A = () => {
  ...
  const handleHapus = () => {
    if (value) {
      ...
      axios
      .post('http://10.0.2.2:105/hapus_sounding', requestData)
      .then((response) => {
        const { message, error } = response.data;
        if (message) {
          ToastAndroid.show(message, ToastAndroid.SHORT);
          navigation.navigate('Sounding_A'); // Navigasi kembali ke halaman Sounding_A
        } else if (error) {
        ...
        .catch((error) => {
          ...
          ToastAndroid.show('Terjadi kesalahan saat menghapus data.', ToastAndroid.SHORT);
        });
    } else {
      console.warn('Pilih bulan terlebih dahulu');
      ToastAndroid.show('Silakan pilih bulan sebelum melakukan penghapusan.', ToastAndroid.SHORT);
    }
  };
